 ...
 */
import { getFunctions, httpsCallable, type FunctionsErrorCode } from "firebase/functions";
...
/** Error codes the function itself raises on purpose to describe a specific
 * book's outcome (not found in the catalog, no plain-text edition, bad
 * input) — these prove the function IS reachable and working, so they
 * should never trip the circuit breaker. Everything else (unavailable,
 * internal, deadline-exceeded, or no `.code` at all because the call
 * never reached the function) genuinely indicates the service is down and
 * should.
 */
const EXPECTED_PER_BOOK_CODES = new Set<FunctionsErrorCode>([
  "not-found",
  "failed-precondition",
  "invalid-argument",
]);
...
function isInfraFailure(err: unknown): boolean {
  ...
  if (!code) return true;
  const short = code.startsWith("functions/") ? code.slice("functions/".length) : code;
  return !EXPECTED_PER_BOOK_CODES.has(short as FunctionsErrorCode);
}
  ...
  if (fb && !pdFnBreaker.isOpen()) {
    try {
      ...
      if (isInfraFailure(err)) {
        ...
        console.warn("[public-domain] cloud function search reported a per-query error", err);
      }
    }
  }
...
export async function getPublicDomainBook(gutenbergId: number): Promise<Book> {
  ...
  if (fb && !pdFnBreaker.isOpen()) {
    try {
      ...
      if (!isInfraFailure(err)) {
        // The function is reachable and gave a definitive, specific answer
        // about this book (not found / no plain-text edition / bad id) —
        // that's authoritative, so surface its own clear message instead
        // of masking it behind a generic fallback attempt, and don't
        // punish every other book's read attempt for the next 5 minutes.
        throw new Error((err as { message?: string }).message || "Não foi possível abrir este livro.");
      }
      pdFnBreaker.trip();
      console.warn(
